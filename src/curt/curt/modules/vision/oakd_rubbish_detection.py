...
class OAKDRubbishDetection(OAKDProcessingWorker):
    def __init__(self):
        super().__init__()
        self.spatial_detection = False

    def preprocess_input(self, params):
        if "rubbish_detection" not in self.oakd_pipeline.xlink_nodes:
            logging.warning("No such node: rubbish_detection in the pipeline")
            return []
        self.od_nn_node_names = self.oakd_pipeline.xlink_nodes["rubbish_detection"]
        operation = params[0]
        if operation == "get_spatial_rubbish_detection":
            self.spatial_detection = True
            return True
        elif operation == "detect_rubbish":
            img = params[1]
            if img is None:
                return None
            ...
            return img
        elif operation == "detect_rubbish_pipeline":
            return True
    ...
    def execute_nn_operation(self, preprocessed_data):
        ...
        if isinstance(preprocessed_data, bool):
            ...
            frame_fd.setWidth(
                self.oakd_pipeline.nn_node_input_sizes["rubbish_detection"][0]
            )
            frame_fd.setHeight(
                self.oakd_pipeline.nn_node_input_sizes["rubbish_detection"][1]
            )
            frame_fd.setData(
                to_planar(
                    preprocessed_data,
